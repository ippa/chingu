...
tweening, movement and even method calls. Instructions are added to a queue to
be processed in order; the operation at the front of the queue is updated each
tick and removed when it has finished.
...
Currently available operations are wait(timeout, &condition), tween(timeout,
properties) and call(method, *arguments).
