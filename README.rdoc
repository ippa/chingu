...
Assuming the below code is included in a class FireBall.
  ...
  # If a fire_ball-10x10.png/bmp exists, it will be loaded as a tileanimation (and fire_ball.png image will be ignored)
	# 10x10 would indicate the width and height of each tile so Chingu knows hows to cut it up into single frames.
	# The below example will animate the frames with 200ms delay between each frame.
