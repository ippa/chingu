...
Am I missing your Chingu project? Msg me on github and I'll add it to the list!
...
For example, inside game state Menu you call push_game_state(Level). When Level exits, it will go back to Menu.
...
=== Various Helpers
Both $window and game states gets some new graphical helpers, currently only 5, but quite useful:
...
... the above is repeated until game exits.
...
* Encapsulate only the very common basics that _most_ in-game objects need
...
An example using almost all arguments would be:
  ...
  # If this is not what you want, use the :draw and :update options:
  #
...
Game states aren't complicated. In Chingu a GameState is a class that behaves mostly like your default Gosu::Window (or in our case Chingu::Window) game loop.
  ...
  class Intro < Chingu::GameState
    ...
    # Called Each time when we activate/enter the game state, use this to reset the gamestate to a "virgin state"
    def setup
      @player.angle = 0   # point player upwards
    end
    
    # Called when we deactivate/leave the game state
    def finalize
...
The bounding_box trait also provides the method cache_bounding_box(). After calling cache_bounding_box() calls to bounding_box() will be quicker but it will not longer adapt to size-changes.
...
The bounding_circle traits provides the method cache_bounding_circle(). After calling cache_bounding_circle() calls to radius() will be quicker but it will not longer adapt to size-changes.
