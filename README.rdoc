...
Roughly 50 lines became 25 more powerfull lines. (you can do @player.angle = 100 for example)

If you've worked with Gosu for a while you're probably tired of passing around the window-argument.
Chingu solves this (as many other developers) with a global variable $window. Yes, globals are bad, but in this case it kinda makes sense.
...
It enables you to start your game from any directory and it will still find your assets correctly.
For a local developmentversion this might not be important, you're likely to start the game from the game root-dir.
